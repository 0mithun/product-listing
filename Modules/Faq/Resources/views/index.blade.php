...
<div class="card">
            ...
            <li class="nav-item mr-3" role="presentation">
                @php
                    ...
                    if( request('category') && !is_null(request('category'))){
                        $active = Str::lower(request('category')) ==   Str::lower($category->name) ? 'active' : '';
                    }else {
                        $active = ($loop->index == 0 ? 'active' : '');
                    }
                @endphp
                <a class="nav-link {{  $active }}" href="{{ route('module.faq.index', ['category'=>$category->slug]) }}" >{{ $category->name }}</a>
            </li>
            @endforeach
        </ul>
    </div>
    <div class="card-body">
        ...
        <div class="col-12" id="accordion">
            ...
            @if (userCan('faq.delete') || userCan('faq.edit'))
                <div class="d-flex justify-content-end">
                    @if (userCan('faq.delete') || userCan('faq.view'))
